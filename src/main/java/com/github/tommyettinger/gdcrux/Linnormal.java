...
/**
 * Like the "Ziggurat algorithm" for generating normal-distributed doubles, but preserves patterns in the input
 * {@code long}. Uses a large table of the results of {@link #probitHighPrecision(double)}, and interpolates between
 * them using linear interpolation. This tends to be faster than Ziggurat at generating normal-distributed values,
 * though it probably has slightly worse quality. Since Ziggurat is already much faster than other common methods, such
 * as the Box-Muller Method, probit() function, or the Marsaglia Polar Method (which Java itself uses), this being
 * faster than Ziggurat is a good thing.
 * <br>
